...
The two primary data tasks stay at the top level. Provider discovery and configuration diagnostics are grouped into secondary operational branches.
...
Libex and AudioSilo publish OpenAPI descriptions. Their normal search, lookup, detail, and health paths use typed clients generated by Kiota. Hand-written mappers translate generated wire models into `SearchResult` without leaking provider model shapes into the public contract.
