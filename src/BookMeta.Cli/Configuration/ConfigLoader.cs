...
{
    public BookMetaConfig Load(string? explicitPath, bool requireFile = true)
    {
        var path = paths.Resolve(explicitPath);
        if (!File.Exists(path))
        {
            if (!requireFile)
                return new BookMetaConfig { SourcePath = path };
        ...
        try
        {
            ...
            root = parsed ?? throw new BookMetaException("Configuration is empty.", ExitCodes.Configuration, "Add version, providers, and groups to the TOML file.");
        }
        catch (TomlException exception)
        {
            throw new BookMetaException(
                $"Configuration is invalid: {exception.Message}", ExitCodes.Configuration,
                "Run 'bookmeta config validate' after correcting the TOML syntax.", exception);
        }
        var config = new BookMetaConfig
        {
            SourcePath = path,
    ...
    private static BookMetaException ConfigError(string message) => new($"Configuration is invalid: {message}.", ExitCodes.Configuration, "Correct the configuration and run 'bookmeta config validate'.");
}
