...
namespace BookMeta.Commands.Author;
    ...
    AppConsole console) : AsyncCommand<AuthorBooksSettings>
{
    ...
    private static ProviderConfig ResolveProvider(BookMetaConfig config, string? id)
    {
        if (id is not null)
        {
            if (!config.Providers.TryGetValue(id, out var selected))
                throw new BookMetaException($"Unknown provider '{id}'.", ExitCodes.Configuration, "Run 'bookmeta providers list'.");
            if (!selected.Enabled)
                throw new BookMetaException($"Provider '{id}' is disabled.", ExitCodes.Configuration, "Enable it before looking up an author.");
            return selected;
        }
        ...
        {
            1 => candidates[0],
            0 => throw new BookMetaException("No enabled Libex provider is configured.", ExitCodes.Configuration, "Configure a Libex provider and retry."),
            _ => throw new BookMetaException("Multiple Libex providers are enabled.", ExitCodes.Usage, "Select one with '--provider ID'.")
        };
    }

    private static BookMetaException Unsupported(string provider)
        => new($"Provider '{provider}' does not support native author-book lookup.", ExitCodes.UnsupportedCapability, "Select a Libex provider or use 'bookmeta search --author NAME'.");
}
