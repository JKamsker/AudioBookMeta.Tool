...
{
    public async Task<SearchExecution> ExecuteAsync(AudiobookMetaConfig config, SearchRequest request, SearchOptions options, CancellationToken cancellationToken)
    {
        var providers = selector.Select(config, options.Includes, options.Groups, options.Excludes);
        if (providers.Count == 0)
            throw new AudiobookMetaException("No enabled providers were selected.", ExitCodes.Configuration, "Enable a provider or select a non-empty provider group.");
