...
public static class SecretResolver
{
    ...
    public static string Resolve(string reference)
    {
        if (!HasValidSyntax(reference))
            throw new AudiobookMetaException("Invalid secret reference syntax.", ExitCodes.Configuration, "Use env:NAME, file:/path, or literal:value.");
        if (reference.StartsWith("env:", StringComparison.Ordinal))
        {
            var name = reference[4..];
            return Environment.GetEnvironmentVariable(name) ?? throw new AudiobookMetaException($"Environment variable '{name}' referenced by auth is not set.", ExitCodes.Configuration, $"Set {name} before running this command.");
        }
        if (reference.StartsWith("file:", StringComparison.Ordinal))
        {
            var path = Environment.ExpandEnvironmentVariables(reference[5..]);
            if (!File.Exists(path))
                throw new AudiobookMetaException($"Secret file was not found: {path}", ExitCodes.Configuration, "Create the file or update the auth reference.");
