...
public static class ConfigValidator
{
    ...
    public static IReadOnlyList<string> Validate(AudiobookMetaConfig config, bool resolveSecrets)
    {
        ...
        foreach (var provider in config.Providers.Values)
        {
            ...
            if (resolveSecrets && provider.Auth is not null)
            {
                ...
                catch (AudiobookMetaException exception) { errors.Add($"provider '{provider.Id}': {exception.Message}"); }
            }
            foreach (var header in provider.Headers)
            {
                ...
                if (resolveSecrets && SecretResolver.HasValidSyntax(header.Value))
                {
                    ...
                    catch (AudiobookMetaException exception) { errors.Add($"provider '{provider.Id}' header '{header.Key}': {exception.Message}"); }
                }
            }
        }
    ...
    public static void ThrowIfInvalid(AudiobookMetaConfig config, bool resolveSecrets)
    {
        var errors = Validate(config, resolveSecrets);
        if (errors.Count > 0)
            throw new AudiobookMetaException($"Configuration is invalid: {string.Join("; ", errors)}.", ExitCodes.Configuration, "Correct the file and run 'dotnet audiobookmeta config validate'.");
    }

    private static void ValidateGroup(AudiobookMetaConfig config, string name, HashSet<string> stack, List<string> errors)
    {
        if (!stack.Add(name))
        {
