...
public sealed class DiagnosticLogger(ConfigPathResolver paths)
{
    public string Write(Exception exception, string[] arguments)
    {
        ...
        var path = Path.Combine(directory, $"audiobookmeta-error-{DateTimeOffset.UtcNow:yyyyMMdd-HHmmss-fff}.log");
        ...
        File.WriteAllText(path,
            ...
            $"command: {ApplicationIdentity.Command} {string.Join(' ', safeArgs)}{Environment.NewLine}" +
