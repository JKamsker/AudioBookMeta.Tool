...
public sealed class ConfigurationTests
{
    ...
    public void Recursive_group_is_rejected()
    {
        var path = TemporaryConfig("""
            ...
            b = ["@a"]
            """);
        var exception = Assert.Throws<BookMetaException>(() => new ConfigLoader(new ConfigPathResolver()).Load(path));
    ...
    public void Public_plain_http_requires_explicit_opt_in()
    {
        var path = TemporaryConfig("""
            ...
            """);
        Assert.Throws<BookMetaException>(() => new ConfigLoader(new ConfigPathResolver()).Load(path));
    }

    private static string TemporaryConfig(string content)
    {
        var path = Path.Combine(Path.GetTempPath(), $"bookmeta-test-{Guid.NewGuid():N}.toml");
